    ...
    var states = d3.json("us.json", function(error, US) {
        ...
        .on('click', function(d){
            ...
            for (var data in pieChartData) {
                ...
                var elementId = energyType.split(' ').join('_');

                document.getElementById(elementId).innerHTML="<b>" + energyType + "</b>: " + 
            ...
            for (var data in yearPieChartData) {

                var energyType = yearPieChartData[data].label;
                var elementId = energyType.split(' ').join('_');

                document.getElementById("n" + elementId).innerHTML="<b>" + energyType + "</b>: " + 
        ...
        for (var prop in d[typePower]) {
            ...
            for (var obj in returnData) {
                ...
                if (returnData[obj].label == prop) {
                    var added = 1;
                    var curValue = parseInt(d[typePower][prop].replace(/,/g, ''));
                    if (curValue > 0) {
            ...
            for (var prop in d[state][typePower]) {
                ...
                for (var obj in returnData) {
                    ...
                    if (returnData[obj].label == prop) {
                        ...
                        var curValue = parseInt(d[state][typePower][prop].replace(/,/g, '')); 
                        if (curValue > 0) {
                            returnData[obj].value += curValue;        
                        }
